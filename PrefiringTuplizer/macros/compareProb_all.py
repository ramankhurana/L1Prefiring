...
def savegraph(rootfilename, name, ymax):
    ...
    g_prob_.GetXaxis().SetRangeUser(0,11)
    g_prob_.GetYaxis().SetRangeUser(0.01,ymax)
    g_prob_.GetXaxis().SetTitle("ADC threshold on matched TT");
    ...
    g_prob_26_.GetXaxis().SetRangeUser(0,10)
    g_prob_26_.GetYaxis().SetRangeUser(0.01,ymax)
    g_prob_26_.GetXaxis().SetTitle("ADC threshold on matched TT");
    ...
    g_prob_27_.GetXaxis().SetRangeUser(0,10)
    g_prob_27_.GetYaxis().SetRangeUser(0.01,ymax)
    g_prob_27_.GetXaxis().SetTitle("ADC threshold on matched TT");
    ...
    g_prob_28_.GetXaxis().SetRangeUser(0,10)
    g_prob_28_.GetYaxis().SetRangeUser(0.01,ymax)
    g_prob_28_.GetXaxis().SetTitle("ADC threshold on matched TT");
    ...
    g_prob_29_.GetXaxis().SetRangeUser(0,10)
    g_prob_29_.GetYaxis().SetRangeUser(0.01,ymax)
    g_prob_29_.GetXaxis().SetTitle("ADC threshold on matched TT");
    ...
    g_prob_29_.Draw("Psame")
    
    
    
    
    leg = TLegend(.50, .731, .90, .890);
    if ybin_==2: leg = TLegend(.50, .731, .90, .890);
    ...
    latex.DrawLatex(0.14, 0.93, "CMS Internal                          #sqrt{s} = 13 TeV (2017)");
...
rootfilename = ["prob_graphs_probability_scan_ADC_Ring.root","prob_graphs_probability_2017_2_2.root"]
pdfname = ["Plots2017_FrontTrain/prefire_prob_compare_etarings","Plots2017_FrontTrain/prefire_rate_compare_etarings"]
ymax=[0.3, 1.0]
for i in range(len(rootfilename)):
    savegraph(rootfilename[i],pdfname[i],ymax[i])
